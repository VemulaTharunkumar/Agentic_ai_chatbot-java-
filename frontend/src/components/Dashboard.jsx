...
const formatMarkdown = (text) => {
  if (!text) return text;
  // Fix tables where the LLM inserts blank lines between rows, which breaks markdown parsers
    ...
    <div className="dashboard-layout">
            ...
            <div className="historical-view animate-fade-in">
              ...
              <div className="history-item">
                ...
                <div className="history-response glass-panel">
                  <div className="response-header" style={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      <div className="avatar bot-avatar-small">🤖</div>
                      <span className="agent-badge">{selectedHistoryItem.agent}</span>
                    </div>
                    <button 
                      className={`copy-btn ${copiedId === 'history' ? 'copied' : ''}`}
                      onClick={() => handleCopyText(selectedHistoryItem.response, 'history')}
                    >
                      {copiedId === 'history' ? <><Check size={14}/> Copied</> : <><Copy size={14}/> Copy</>}
                    </button>
                  </div>
                  <div className="response-content markdown-body">
                    <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw]}>{formatMarkdown(selectedHistoryItem.response)}</ReactMarkdown>
                  </div>
                </div>
                
                {/* Expandable Output Details for History */}
                {(() => {
                  let parsedOutputs = {};
                  try {
                    if (selectedHistoryItem.outputsJson) {
                      parsedOutputs = JSON.parse(selectedHistoryItem.outputsJson);
                    }
                  } catch (e) {}
                        ...
                        {expandedSection && (
                          <div className="details-grid animate-fade-in">
                            {['planner', 'research', 'coder', 'critic'].map((agentName) => {
                              const output = parsedOutputs[agentName];
            ...
            <div className="results-container animate-fade-in">
              ...
              <div className="final-answer-box glass-panel">
                <div className="box-header">
                  ...
                  <button 
                    className={`copy-btn ${copiedId === 'final' ? 'copied' : ''}`}
                    onClick={() => handleCopyText(result.final_answer, 'final')}
                  >
                    {copiedId === 'final' ? <><Check size={16}/> Copied</> : <><Copy size={16}/> Copy Output</>}
                  </button>
                </div>
                
                <div className="answer-content">
                  <div className="markdown-body">
                    <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw]}>{formatMarkdown(result.final_answer)}</ReactMarkdown>
                  </div>
                </div>
              </div>
      ...
      {popupAgent && (
        ...
        >
          ...
          >
            <div className={`detail-header modal-header ${popupAgent.name}`} style={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <button 
                  className={`copy-btn ${copiedId === 'modal' ? 'copied' : ''}`}
                  onClick={() => handleCopyText(popupAgent.output, 'modal')}
                  style={{ padding: '0.2rem 0.5rem', fontSize: '0.8rem' }}
                >
                  {copiedId === 'modal' ? <><Check size={14}/> Copied</> : <><Copy size={14}/> Copy</>}
                </button>
                <button 
            ...
            <div className="modal-body detail-body markdown-body">
              <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw]}>{formatMarkdown(popupAgent.output)}</ReactMarkdown>
            </div>
          </div>
        </div>
      )}
